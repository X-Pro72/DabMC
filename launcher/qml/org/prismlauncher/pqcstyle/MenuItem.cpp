...
PStyleMenuItem::PStyleMenuItem(QQuickItem* parent) : PQuickStyleItem(parent)
{
    m_type = QStringLiteral("menuitem");
}

void PStyleMenuItem::doInitStyleOption()
{
    if (!m_styleoption) {
    ...
    setProperty("_q_isComboBoxPopupItem", false);
    ...
    if (type == PQuickStyleItem::ScrollIndicatorType) {
        ...
        const QFont font = qApp->font("QMenu");
        opt->font = font;
...
QSize PStyleMenuItem::getContentSize(int width, int height)
{
    QSize size;
...
void PStyleMenuItem::doPaint(QPainter* painter)
{
