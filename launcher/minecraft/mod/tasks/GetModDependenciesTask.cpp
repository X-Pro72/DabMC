                                                        ...
                                                        int level)
{
    ...
    callbacks.on_succeed = [dep, provider, pDep, level, this](auto& pack) {
        ...
        if (dep.addonId.toString().isEmpty() && !pDep->version.addonId.toString().isEmpty()) {
            ...
            auto dep = getOverride({ .addonId = pDep->version.addonId, .type = pDep->dependency.type, .version = "" }, provider);
            if (dep.addonId != pDep->version.addonId) {
                removePack(pDep->version.addonId);
                addTask(prepareDependencyTask(dep, provider, level));
            } else {
        ...
        for (const auto& dep : getDependenciesForVersion(pDep->version, provider)) {
            addTask(prepareDependencyTask(dep, provider, level - 1));
        }
    };
